...
const fmtDept=d=>d==='systems'?'Sistemas':d==='maintenance'?'Mantenimiento':d||'—';
...
window.assignRequest=async id=>{
  ...
  try{
    const d=await api(`/api/requests/${id}/assign`,{method:'POST',body:'{}'});
    toast(`Ticket ${d.ticket.ticket_number} asignado`,'success');
    await refreshCurrent();
  }catch(e){
    toast(e.message,'error');
    buttons.forEach(b=>{b.disabled=false;b.textContent=b.dataset.oldText||'Asignarme'});
  }
};
...
function ticketActions(x){
  ...
  return opts.length
    ? `<div class="ticket-actions"><select id="st-${x.id}"><option value="" selected disabled>Seleccionar estado</option>${opts.map(s=>`<option value="${s}">${statusNames[s]}</option>`).join('')}</select><button class="btn small primary" onclick="changeTicket(${x.id})">Aplicar</button><button class="btn small secondary" onclick="ticketDetail(${x.id})">Detalle</button></div>`
    : `<button class="btn small secondary" onclick="ticketDetail(${x.id})">Detalle</button>`;
}
...
async function ensureReportFilters(){if(reportFilters)return;reportFilters=await api('/api/report-filters');$('#repGroup').innerHTML='<option value="">Todos</option>'+reportFilters.groups.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('');$('#repEngineer').innerHTML='<option value="">Todos</option>'+reportFilters.engineers.map(x=>`<option value="${x.id}">${esc(x.full_name)} · ${x.role==='superadmin'?'Superadmin':x.role==='admin'?'Admin':fmtDept(x.department)}</option>`).join('');$('#repStatus').innerHTML='<option value="">Todos</option>'+reportFilters.statuses.map(x=>`<option value="${x}">${statusNames[x]||x}</option>`).join('');updateReportCategories();if(me.role==='engineer'){$('#repDept').value=me.department;$('#repDept').disabled=true}}
function updateReportCategories(){if(!reportFilters)return;const d=$('#repDept').value||me?.department||'';$('#repCategory').innerHTML='<option value="">Todas</option>'+reportFilters.categories.filter(x=>!d||x.department===d).map(x=>`<option value="${x.code}">${fmtDept(x.department)} · ${esc(x.label)}</option>`).join('')}
...
async function loadReports(init=false){
  ...
  const qs=new URLSearchParams({from:$('#repFrom').value,to:$('#repTo').value,department:$('#repDept').value,groupId:$('#repGroup').value,category:$('#repCategory').value,engineerId:$('#repEngineer').value,status:$('#repStatus').value});
...
function reportCsv(){
  ...
  const d=reportCache,rows=[['ANDON Support - Reporte'],[],['Métrica','Valor'],['Tickets creados',d.summary.total],['Tickets cerrados',d.summary.closed],['Promedio respuesta',duration(d.summary.avg_response_seconds)],['Promedio resolución',duration(d.summary.avg_resolution_seconds)],['SLA respuesta',pct(d.summary.response_met,d.summary.response_measured)+'%'],['SLA resolución',pct(d.summary.resolution_met,d.summary.resolution_measured)+'%'],[],['Técnico','Asignados','Cerrados','Pendientes','Resp. prom.','Resol. prom.','Cumplimiento de cierre','SLA resolución']];
  (d.engineerPerformance||[]).forEach(x=>rows.push([x.label,x.total,x.closed,x.pending,duration(x.avg_response_seconds),duration(x.avg_resolution_seconds),x.closure_percentage+'%',x.sla_percentage==null?'':x.sla_percentage+'%']));
  rows.push([],['Equipo','Grupo','Tickets','Incidencia principal','Resol. prom.']);
  (d.recurrent||[]).forEach(x=>rows.push([x.label,x.group_name,x.total,x.principal_incident||'',duration(x.avg_resolution_seconds)]));
  const csv='\ufeff'+rows.map(r=>r.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(',')).join('\r\n');
  const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`Andon-Reporte-${$('#repFrom').value}-${$('#repTo').value}.csv`;a.click();URL.revokeObjectURL(a.href);
}
...
function renderAdminStations(){
  ...
  selectedAdminGroupId=gid||null;
  const rows=(adminData.stations||[]).filter(st=>!gid||Number(st.group_id)===gid);
  $('#stationsAdminList').innerHTML=rows.length
    ? rows.map(st=>`<div class="admin-item"><div><strong>${esc(st.code)}</strong><small>${esc(st.group_name||'')} · ${esc(st.label||'')} · ${st.enabled?'Activo':'Deshabilitado'} · /station/${esc(st.code)}</small></div><div class="admin-actions"><button class="btn small secondary" onclick="renameStation(${st.id},'${esc(st.label||'').replace(/'/g,"\\'")}')">Renombrar</button><button class="btn small ${st.enabled?'danger':'resolve'}" onclick="toggleStation(${st.id},${!st.enabled})">${st.enabled?'Deshabilitar':'Habilitar'}</button><button class="btn small danger" onclick="deleteStation(${st.id})">Eliminar</button></div></div>`).join('')
    : '<div class="empty">No hay equipos en el grupo seleccionado.</div>';
}

async function loadGroups(preferredGroupId=null){
  const before=preferredGroupId||selectedAdminGroupId||Number($('#stationGroup')?.value||0)||null;
  adminData=await api('/api/admin/groups');
  const enabledGroups=adminData.groups.filter(x=>x.enabled);
  $('#stationGroup').innerHTML=enabledGroups.map(g=>`<option value="${g.id}">${esc(g.name)}</option>`).join('');

  let target=Number(before||0);
  if(!enabledGroups.some(g=>Number(g.id)===target)) target=enabledGroups.length?Number(enabledGroups[0].id):0;
  if(target){$('#stationGroup').value=String(target);selectedAdminGroupId=target}else selectedAdminGroupId=null;

  $('#groupsAdminList').innerHTML=adminData.groups.map(g=>`<div class="admin-item"><div><strong>${esc(g.name)}</strong><small>${esc(g.code)} · ${g.enabled?'Activo':'Deshabilitado'}</small></div><div class="admin-actions"><button class="btn small secondary" onclick="renameGroup(${g.id},'${esc(g.name).replace(/'/g,"\\'")}')">Renombrar</button><button class="btn small ${g.enabled?'danger':'resolve'}" onclick="toggleGroup(${g.id},${!g.enabled})">${g.enabled?'Deshabilitar':'Habilitar'}</button><button class="btn small danger" onclick="deleteGroup(${g.id})">Eliminar</button></div></div>`).join('');

  renderAdminStations();
}

$('#stationGroup').addEventListener('change',()=>{
  selectedAdminGroupId=Number($('#stationGroup').value||0)||null;
  renderAdminStations();
});
...
$('#groupForm').addEventListener('submit',async e=>{
  e.preventDefault();
  try{
    const d=await api('/api/admin/groups',{method:'POST',body:JSON.stringify({name:$('#groupName').value.trim()})});
    toast(`Grupo ${d.name} creado`,'success');
    $('#groupName').value='';
    await loadGroups(d.id);
  }catch(x){toast(x.message,'error')}
});

$('#stationForm').addEventListener('submit',async e=>{
  e.preventDefault();
  const gid=Number($('#stationGroup').value||0);
  const name=$('#stationName').value.trim();
  try{
    const d=await api('/api/admin/stations',{method:'POST',body:JSON.stringify({groupId:gid,name})});
    toast(`Equipo ${d.code} creado`,'success');
    $('#stationName').value='';
...
window.renameGroup=async(id,current)=>{
  const name=prompt('Nuevo nombre del grupo:',current);
  if(!name||name.trim()===current.trim())return;
  try{
    await api(`/api/admin/groups/${id}`,{method:'PATCH',body:JSON.stringify({name:name.trim()})});
    toast('Grupo renombrado','success');
    await loadGroups(selectedAdminGroupId);
  }catch(e){toast(e.message,'error')}
};

window.renameStation=async(id,current)=>{
  const name=prompt('Nuevo nombre del equipo:',current);
  if(!name||name.trim()===current.trim())return;
  try{
    const d=await api(`/api/admin/stations/${id}`,{method:'PATCH',body:JSON.stringify({name:name.trim()})});
    toast(`Equipo renombrado: ${d.code}`,'success');
    await loadGroups(selectedAdminGroupId);
  }catch(e){toast(e.message,'error')}
};
...
window.deleteGroup=async id=>{if(!confirm('¿Archivar este grupo y sus equipos? El historial no se elimina.'))return;try{await api(`/api/admin/groups/${id}`,{method:'DELETE'});selectedAdminGroupId=null;await loadGroups()}catch(e){toast(e.message,'error')}};
window.deleteStation=async id=>{if(!confirm('¿Archivar este equipo? El historial no se elimina.'))return;try{await api(`/api/admin/stations/${id}`,{method:'DELETE'});await loadGroups(selectedAdminGroupId)}catch(e){toast(e.message,'error')}};

function userRoleName(role){return role==='superadmin'?'Super Administrador':role==='admin'?'Administrador':role==='engineer'?'Ingeniero':role}
function userAreaName(u){return u.role==='engineer'?(u.department?fmtDept(u.department):'—'):'Todas'}
async function loadUsers(){const rows=await api('/api/admin/users');$('#usersTable').innerHTML=rows.map(u=>`<tr><td>${esc(userAreaName(u))}</td><td>${esc(userRoleName(u.role))}</td><td><strong>${esc(u.username)}</strong></td><td>${esc(u.full_name)}</td><td>${u.active?'Activo':'Deshabilitado'}</td><td>${userActions(u)}</td></tr>`).join('')}
...
function syncUserCreateFields(){const role=$('#userRole').value;const dep=$('#userDepartment');if(role==='engineer'){dep.disabled=false;if(!['systems','maintenance'].includes(dep.value))dep.value='systems';dep.title='Área que determina el pool de solicitudes del ingeniero';}else{dep.disabled=true;dep.title='Administradores tienen acceso global (Todas las áreas)';}}
$('#userRole').addEventListener('change',syncUserCreateFields);
$('#userForm').addEventListener('submit',async e=>{e.preventDefault();const role=$('#userRole').value;try{await api('/api/admin/users',{method:'POST',body:JSON.stringify({department:role==='engineer'?$('#userDepartment').value:'',role,username:$('#userUsername').value,fullName:$('#userFullName').value,password:$('#userPassword').value})});toast('Usuario creado','success');e.target.reset();syncUserCreateFields();loadUsers()}catch(x){toast(x.message,'error')}});
window.editUser=async(id,name,role,department)=>{const fullName=prompt('Nombre:',name);if(!fullName)return;const newRole=prompt('Rol: admin o engineer',role);if(!newRole)return;let dep=department;if(newRole==='engineer')dep=prompt('Área: systems o maintenance',department||'systems')||department;try{await api(`/api/admin/users/${id}`,{method:'PATCH',body:JSON.stringify({fullName,role:newRole,department:dep})});loadUsers()}catch(e){toast(e.message,'error')}};window.resetUser=async id=>{const password=prompt('Nueva contraseña (mínimo 6 caracteres):','');if(!password)return;try{await api(`/api/admin/users/${id}/reset-password`,{method:'POST',body:JSON.stringify({password})});toast('Contraseña restablecida','success')}catch(e){toast(e.message,'error')}};window.deleteUser=async id=>{if(!confirm('¿Eliminar/desactivar usuario? Su historial permanecerá.'))return;try{await api(`/api/admin/users/${id}`,{method:'DELETE'});loadUsers()}catch(e){toast(e.message,'error')}};
...
$('#slaForm').addEventListener('submit',async e=>{e.preventDefault();try{await api('/api/admin/slas',{method:'POST',body:JSON.stringify({name:$('#slaName').value,department:$('#slaDept').value,groupId:$('#slaGroup').value,stationId:$('#slaStation').value,category:$('#slaCategory').value,responseMinutes:$('#slaResponse').value,resolutionMinutes:$('#slaResolution').value,autocloseMinutes:$('#slaAutoclose').value,priority:$('#slaPriority').value,pauseOnWaiting:$('#slaPause').checked})});toast('SLA creado','success');e.target.reset();$('#slaResponse').value=5;$('#slaResolution').value=60;$('#slaAutoclose').value=10;$('#slaPriority').value=100;$('#slaPause').checked=true;loadSla()}catch(x){toast(x.message,'error')}});
function ensureSlaEditModal(){
  ...
  el.innerHTML=`<div class="modal-card sla-edit-card">
    ...
    <form id="slaEditForm" class="sla-edit-form">
      ...
      <label>Prioridad<input type="number" min="1" id="slaEditPriority" required></label>
...
window.editSla=(id)=>{
  ...
  $('#slaEditResponse').value=s.response_minutes;$('#slaEditResolution').value=s.resolution_minutes;$('#slaEditAutoclose').value=s.autoclose_minutes;$('#slaEditPriority').value=s.priority||100;$('#slaEditPause').checked=!!s.pause_on_waiting;
  $('#slaEditModal').classList.remove('hidden');
};
async function saveSlaEdit(e){
  e.preventDefault();const id=$('#slaEditId').value;
  const body={name:$('#slaEditName').value,department:$('#slaEditDept').value,groupId:$('#slaEditGroup').value,stationId:$('#slaEditStation').value,category:$('#slaEditCategory').value,responseMinutes:$('#slaEditResponse').value,resolutionMinutes:$('#slaEditResolution').value,autocloseMinutes:$('#slaEditAutoclose').value,priority:$('#slaEditPriority').value,pauseOnWaiting:$('#slaEditPause').checked};
...
function renderIdentity(){if(!me)return;const role=me.role==='superadmin'?'Super Administrador':me.role==='admin'?'Administrador':`Ingeniero ${fmtDept(me.department)}`;$('#roleLabel').textContent=`${me.fullName} · ${role}`;$('#sideUser').textContent=me.fullName;$$('.manager-only').forEach(x=>x.classList.toggle('hidden',!['superadmin','admin'].includes(me.role)));if(me.role!=='superadmin')$('#superadminOption')?.remove()}
...
['repFrom','repTo','repDept','repGroup','repCategory','repEngineer','repStatus'].forEach(id=>{
  const el=$('#'+id);
...
$('#exportReportExcel').addEventListener('click',reportCsv);
